# 🏸 バドミントン スタッツ記録（得点カウンターへ統合済み）

> **このアプリは「バドミントン得点カウンター」(`badminton-scorer`) に統合されました。**
> 今後はそちらのホームで「スタッツも記録」を選んで使ってください。スタッツ共有チームの合言葉・履歴データ（Firebase の `stats/`）はそのまま引き継がれます。このリポジトリは記録として残しています。
>
> 統合先: https://github.com/gusa-laboratory/badminton-scorer
